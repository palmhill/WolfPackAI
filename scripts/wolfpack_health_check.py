...
class WolfPackHealthChecker:
    ...
    def __init__(
        self,
        config_path: str = "mod_squad.config.json",
        wait: bool = False,
        timeout: int = 120,
    ):
    ...
    def check_postgres(self, service_config: dict) -> Tuple[bool, float, str]:
        ...
        try:
            ...
            return (
                False,
                elapsed,
                "pg_isready command not found (PostgreSQL client not installed)",
            )
        ...
        try:
            ...
            if response.status_code == expected_status:
                return (
                    True,
                    elapsed,
                    f"{service_name} is healthy (HTTP {response.status_code})",
                )
            else:
                return (
                    False,
                    elapsed,
                    f"{service_name} returned HTTP {response.status_code}, expected {expected_status}",
                )
            ...
            return (
                False,
                elapsed,
                f"{service_name} connection refused (service not started or wrong port)",
            )
            ...
            return (
                False,
                elapsed,
                f"{service_name} health check timed out after {timeout_s}s",
            )
        except Exception as e:
            elapsed = (time.time() - start) * 1000
    ...
    def check_all_services(self) -> bool:
        ...
        for service_name, service_config in services.items():
            ...
            healthy, elapsed_ms, message = self.check_service(
        ...
        if timings:
            ...
            self.results["p95_ms"] = (
                round(timings[p95_idx], 2)
                if p95_idx < len(timings)
                else round(timings[-1], 2)
            )
            self.results["p99_ms"] = (
                round(timings[p99_idx], 2)
                if p99_idx < len(timings)
                else round(timings[-1], 2)
            )
...
def main():
    ...
    parser.add_argument(
        "--wait", action="store_true", help="Wait for services to become healthy"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick check (critical services only)"
    )

    args = parser.parse_args()

    checker = WolfPackHealthChecker(
        config_path=args.config, wait=args.wait, timeout=args.timeout
    )
